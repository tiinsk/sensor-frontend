...
class DeviceSummaryTemperature extends Component {
  render() {
    const roundedTemp = `${
      this.props.temperature ? this.props.temperature.toFixed(1) : ' -- '
    } °C`;
    const roundedMaxTemp = `${
      this.props.maxTemperature ? this.props.maxTemperature.toFixed(1) : ' -- '
    } °C`;
    const roundedMinTemp = `${
      this.props.minTemperature ? this.props.minTemperature.toFixed(1) : ' -- '
    } °C`;

    const timeDiff = Math.abs(
      DateTime.fromISO(this.props.timestamp).diffNow('minutes').minutes
    );

    let warning = '';

    const timestamp = DateTime.fromISO(this.props.timestamp).toFormat(
      'ccc dd.LL.yy HH:mm'
    );
    ...
    if (timeDiff > 720) {
      ...
      warningTimeStamp = `Temperature captured ${-DateTime.fromISO(
    ...
    return (
      <StyledDeviceTempDiv>
        <StyledWarning title={warningTimeStamp}>{warning}</StyledWarning>
