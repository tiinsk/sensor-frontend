...
export const customPointMark =
  ...
  ({ d, xy, yScale }) => {
    ...
    if (minutes) {
      return (
        <g className="half-hour-mark">
          <rect fill="#40613C" opacity={0.45} width={3} height={y} x={-1} />
          <circle fill={color(d.temperature)} stroke="none" r={4} />
        </g>
      );
    }
    ...
    return (
      <g>
        <text className="unit-text" x={-10} y={-20}>
          {d.temperature.toFixed(1)}
        </text>
        <rect fill="#40613C" width={3} height={y} x={-1} />
        <circle
