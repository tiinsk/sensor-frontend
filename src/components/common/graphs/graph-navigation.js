...
const StyledNavigationLeft = styled.div`
  ...
  &:hover {
    background: ${props =>
      props.theme.colors.blueGraphNavigationBackground_Hover};
  }
...
const StyledGraphWrapper = styled.div`
  ...
  transition: ${props =>
    props.dragging ? 'none' : `left ${ANIMATION_DURATION_MS}ms ease-out`};
`;
...
class GraphNavigation extends Component {
  constructor(props) {
    ...
    this.debouncedPosXListener = throttle(this.setPosX, 50, {
      trailing: false,
    });
  }
  ...
  componentDidMount() {
    ...
    this.navRef.current.addEventListener(
      'touchmove',
      this.debouncedPosXListener,
      { passive: true }
    );

    this.navRef.current.addEventListener('touchend', this.touchEnd, {
      passive: true,
    });
  }
  ...
  clickGraph() {
    ...
    if (this.state.loading || width > variables.theme.breakpointMobileValue)
      return;
  ...
  render() {
    return (
      <StyledGraphNavigation height={this.props.height} ref={this.navRef}>
        <StyledLoadMore
            ...
            <div className="content-wrapper">
              ...
              <div className="date-wrapper">
                ...
                <div className="load-text">
                  <div>
                    {DateTime.fromISO(this.props.startTime).toFormat(
                      'ccc d.L.yyyy'
                    )}
                  </div>
                  <div>
